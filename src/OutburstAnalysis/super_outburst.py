...
filtered_data_points = utils.filter_data_points_super_outburst(
    data_points, lower_limit=SO_BOUNDARY, upper_limit=Q_BOUNDARY
)
...
for index, point in enumerate(filtered_data_points):
    ...
    is_outburst = point["magnitude"] <= SO_BOUNDARY
    is_first_outburst = is_outburst and prev["magnitude"] >= SO_BOUNDARY
    is_last_outburst = is_outburst and next["magnitude"] >= SO_BOUNDARY
